...
export const playtimeService = {
  ...
  async createTrack(playlistId, track) {
    const res = await axios.post(`${this.playtimeUrl}/api/playlists/${playlistId}/tracks`, track);
    return res.data;
  },
  ...
  async authenticate(user) {
    const res = await axios.post(`${this.playtimeUrl}/api/users/authenticate`, user);
